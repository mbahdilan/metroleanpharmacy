  ...
  useEffect(() => {
    async function fetchProduct() {
      try {
        ...
        console.error('Unexpected error in PDP fetch:', err);
      } finally {
        setLoading(false);
      }
    }
          ...
          <div className="pdp-controls-row">
            ...
            <div className="pdp-control-group">
              ...
              <div className="pdp-qty-selector">
                <button className="pdp-qty-btn" onClick={() => setQuantity(Math.max(product.min_quantity || 1, quantity - 1))}>−</button>
                <span className="pdp-qty-value">{quantity}</span>
                <button className="pdp-qty-btn" onClick={() => setQuantity(quantity + 1)}>+</button>
              </div>
              {product.min_quantity > 1 && (
                <span style={{ fontSize: '0.7rem', color: 'var(--accent)', fontWeight: 700, marginTop: '0.4rem', display: 'block' }}>
